import { validateDeleteProduct } from '@/utils/productValidate';
  ...
  return (
    <>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                          onClick={() => setConfirmDialog({ open: true, product })}
      ...
      <DeleteDialog
        ...
        onOpenChange={open => setConfirmDialog({ open, product: open ? confirmDialog.product : null })}
        onConfirm={async () => {
          if (confirmDialog.product) {
            const result = await validateDeleteProduct(confirmDialog.product._id || confirmDialog.product.id);
            if (!result.canDelete) {
              const msg = result.error || 'Không thể xóa sản phẩm này do lỗi hệ thống hoặc dữ liệu.';
              toast.error(msg, { position: 'bottom-right', duration: 3500 });
            } else {
              handleDeleteProduct(confirmDialog.product);
            }
          }
          setConfirmDialog({ open: false, product: null });
        }}
        title="Xóa sản phẩm này?"
        description="Bạn có chắc chắn muốn xóa sản phẩm này không? Thao tác này không thể hoàn tác."
      />
    </>
  );
};
